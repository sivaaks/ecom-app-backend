...
const {addOrder,deleteOrder,updateOrder,getOrdersByType}=require('../shared/validation');
...
const events={
    ...
    async deleteOrder(req,res){
        try{
            ...
            const {user:{_id}}=getTokenDetails(req.headers.auth);
            const data= await db.orders.deleteOne({_id:ObjectId(value.id),userId:_id});
   ...
   async updateOrder(req,res){
        try{
            ...
            const {user:{_id}}=getTokenDetails(req.headers.auth);
            const data= await db.orders.findOneAndUpdate({_id:ObjectId(id),userId:_id},{$set:{...value}});
            return res.status(200).send(data);
        }catch(err){
    ...
    async getOrderByNo(req,res){
        ...
        try{
            const orders= await db.orders.findOne({no:parseInt(orderNo),userId:_id});
    ...
    async getOrderById(req,res){
        ...
        console.log(id);
        try{
            const orders= await db.orders.findOne({_id:ObjectId(id),userId:_id},{_id:0,userId:0});
            console.log('orders',orders);
            return res.status(200).send(orders);
        }catch(err){ 
            console.log(`Error getting orders ${err}`);
   ...
   async getOrdersByType(req,res){
        try{
            const {error,value}= getOrdersByType.validate({type:req.params.type});
            if(error) return res.status(400).send({
                error:'Validation error',
                message:error.details[0].message,
                })
            const {user:{_id}}=getTokenDetails(req.headers.auth);
            const data= await db.orders.find({type:value.type,userId:_id}).sort({date:1}).toArray();
            return res.status(200).send(data);
        }catch(err){
    ...
    async getOrdersByMonth(req,res){
        try{
            ...
            //     })
            const {user:{_id}}=getTokenDetails(req.headers.auth);
            const data= await db.orders.find({userId:_id}).sort({date:1}).toArray();
            return res.status(200).send(data);
        }catch(err){
            console.log(`Get events by type error: ${err}`);
        }
    ...
    async statusUpdate(req,res){
        const {user:{_id}}=getTokenDetails(req.headers.auth);
        try{
            const data=await db.orders.findOneAndUpdate({_id:ObjectId(req.query.id),userId:_id},{$set:{status:req.query.status}});
            console.log(data);
    ...
    async cancelOrder(req,res){
        const {user:{_id}}=getTokenDetails(req.headers.auth);
        try{
            const data=await db.orders.findOneAndUpdate({_id:ObjectId(req.params.id),userId:_id},{$set:{status:'Cancelled'}});
            console.log(data);
    ...
    async getOrderNo(req,res){
        try{
            const {user:{_id}}=getTokenDetails(req.headers.auth);
            const data= await db.orders.find({userId:_id}).sort({no:-1}).limit(1).toArray();
            console.log(data);
            return res.status(200).send(data);
        }catch(err){
